...
export default function AdvisedInvestments({ fundOptions }: AdvisedInvestmentsProps) {
  ...
  const resolveFundCodes = useCallback(async (funds: FundRecommendation[]) => {
    ...
    const updated = [...funds];
    await Promise.all(updated.map(async (fund) => {
        try {
            ...
            if (Array.isArray(json) && json.length > 0) {
              ...
              const filtered = json.filter((item: any) => {
                 ...
                 return (isDirect ? matchesDirect : !matchesDirect) && (isGrowth ? matchesGrowth : true);
              });
              
              const match = filtered.find((item: any) => 
                item.schemeName.toLowerCase().includes(fund.amc?.toLowerCase() || "")
              ) || filtered[0] || json[0];
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const initialFunds = Object.entries(fundOptions).flatMap(([category, funds]) => 
          funds.map(fund => ({ ...fund, category: category.replace(/_/g, " ") }))
        );
        ...
        if (typeof funds === "object" && !Array.isArray(funds)) {
          Object.entries(funds).forEach(([key, d]: [string, any]) => {
            ...
            if (d.isin) dataMap[d.isin.toUpperCase()] = dataMap[key.toUpperCase()];
          });
        }

        setMarketData(dataMap);
      } catch (err) {
        console.error("Failed to fetch market data for advised investments:", err);
      } finally {
        setLoading(false);
      }
    };
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Top-rated assets selected for your growth</p>
        </div>
        <Link 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {resolvedFunds.slice(0, 6).map((fund, i) => {
                  const match = marketData[fund.name.toUpperCase()] || 
                  ...
                  if (fund.schemeCode) {
                    ...
                    const shortName = fund.name.split(" ")[0].toUpperCase().replace(/[^A-Z]/g, "");
          ...
          return (
            <Card key={i} className="group hover:border-primary/50 transition-all overflow-hidden bg-card/50 backdrop-blur-sm">
                ...
                <div className="flex items-start justify-between gap-2">
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <Link href={href}>
                        <CardTitle className={`text-sm font-semibold truncate transition-colors hover:text-primary cursor-pointer`}>
                          {fund.name}
                        </CardTitle>
                      </Link>
                  ...
                  {href !== "#" && (
                    <Link href={href} className="shrink-0 text-muted-foreground hover:text-primary transition-colors">
              ...
              <CardContent className="p-4 pt-0 space-y-3">
                <div className="flex items-end justify-between">
                  <div>
                    <p className="text-[10px] text-muted-foreground uppercase font-bold tracking-tighter">Risk Type</p>
                    <div className="flex items-baseline gap-2">
                      <span className={`text-sm font-bold capitalize py-0.5 px-2 rounded-lg border ${getRiskColor(fund.risk_level)}`}>
                        {fund.risk_level?.replace(/_/g, " ") || "—"}
                      </span>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="text-[10px] text-muted-foreground uppercase font-bold tracking-tighter">Expected Return</p>
                    <p className="text-sm font-bold text-primary">{fund.expected_return_pct}% p.a.</p>
                  </div>
                </div>

                <div className="grid grid-cols-3 gap-2 py-2 border-y border-border/30">
                  <div className="text-center">
                    <p className="text-[9px] text-muted-foreground uppercase font-bold">1Y Ret</p>
                    <p className="text-xs font-semibold text-emerald-400">{fund.returns_1y_pct}%</p>
                  </div>
                  <div className="text-center border-x border-border/30">
                    <p className="text-[9px] text-muted-foreground uppercase font-bold">3Y Ret</p>
                    <p className="text-xs font-semibold text-emerald-400">{fund.returns_3y_pct}%</p>
                  </div>
                  <div className="text-center">
                ...
                <div className="flex items-center justify-between">
                  {fund.isin && (
                    <span className="text-[10px] font-mono text-muted-foreground/50">{fund.isin}</span>
                  )}
                  <div className="flex items-center gap-1.5 grayscale opacity-50">
                    <Badge variant="outline" className="text-[9px] px-1.5 py-0 h-4 bg-muted/20">SIP</Badge>
                    <Badge variant="outline" className="text-[9px] px-1.5 py-0 h-4 bg-muted/20">LONG</Badge>
                  </div>
                </div>
              </CardContent>
            </Card>
